...
pygame.display.set_caption("Alerta covid")
...
class personaje(object):
	def __init__(self, x, y, fuente, limite):
		...
		# Caracteristicas del personaje_principal
		self.va_izquierda = False 
		self.va_derecha = False
		self.contador_pasos = 0
		self.camina_derecha = [pygame.image.load("img/sprite_2.png"), pygame.image.load("img/sprite_3.png")]	
		self.camina_izquierda = [pygame.image.load("img/2.png"), pygame.image.load("img/3.png")]
		self.quieto = pygame.image.load("img/sprite_1.png")
	...
	def dibujar(self, cuadro):
		# Vida del personaje_principal en pantalla (actualizada)
		texto=fuente.render(str(self.salud), True, negro)
		if self.contador_pasos + 1 > 4:
			self.contador_pasos = 0

		if self.va_izquierda:
			cuadro.blit(self.camina_izquierda[self.contador_pasos//2], (self.x,self.y))
			self.contador_pasos += 1
		elif self.va_derecha:
			cuadro.blit(self.camina_derecha[self.contador_pasos//2], (self.x,self.y))
			self.contador_pasos += 1
		else:
		...
		# Barra de vida del personaje_principal
		pygame.draw.rect(cuadro, (255,0,0), (30, 30, 100, 20))
		pygame.draw.rect(cuadro, (0,128,0), (30, 30 , self.salud, 20))

		# Mostrar la vida restante del personaje_principal
		ventana.blit(texto, (55, 19))
	
	# Movimiento del personaje_principal segun teclas
...
class enemigo(object):
	def __init__(self, x, y, fuente, limite):
		...
		self.va_derecha = False
		self.contador_pasos = 0
		self.camina_derecha = [pygame.image.load("img/copiae.png")]
		self.camina_izquierda = [pygame.image.load("img/copiae.png")]
		self.quieto = pygame.image.load("img/copiae.png")
		self.ancho = self.quieto.get_width()
		self.alto = self.quieto.get_height()
...
class enemigo_3(object):
	def __init__(self, x, y, fuente, limite):   
		...
		self.va_derecha = False
		self.contador_pasos = 0
		self.camina_derecha = [pygame.image.load("img/enemigo3.png"), pygame.image.load("img/enemigo3.png"),pygame.image.load("img/enemigo3.png"),pygame.image.load("img/enemigo3.png")]
		self.camina_izquierda = [pygame.image.load("img/enemigo3.png"), pygame.image.load("img/enemigo3.png"),pygame.image.load("img/enemigo3.png"),pygame.image.load("img/enemigo3.png")]
		self.quieto = pygame.image.load("img/enemigo3.png")
		self.ancho = self.quieto.get_width()
		self.alto = self.quieto.get_height()
		# Limites de movimiento
		self.camino = [120, limite]
...
class proyectil(object):
	...
	def __init__(self, x,y,radio,color, direccion):
		...
		self.imagen = [pygame.image.load("img/agujaarriba.png"), pygame.image.load("img/agujaarriba.png")]
...
def subir_nivel():
	...
	if villano[nivel] == villano[2]:
		lvl+=1
		ventana.blit(marcador, (250 - (marcador.get_width()//3), 200))
	
	if villano[nivel] == villano[5]:
		lvl+=1
		ventana.blit(marcador, (250 - (marcador.get_width()//3), 200))
...
while repetir:
	...
	nivel = 0
	nivel_maximo = 5
	imagen_fondo = [pygame.image.load('img/personal.png'), pygame.image.load('img/personal.png'), pygame.image.load('img/personal.png'), pygame.image.load('img/personal.png'), pygame.image.load('img/personal.png'),pygame.image.load('img/personal.png'),pygame.image.load('img/personal.png')]
	puntaje = 0
	...
	# Dibujar personaje_principal
	heroe=personaje(int(30), int(325),"heroe", ventana_x)
	villano = [enemigo_2(int(600), int(345),"villano",int(720)), #cambiar el 385 para la pocicion eje y
				enemigo_2(int(600), int(385),"villano",int(720)), 
				enemigo(int(300), int(345),"villano",int(720)),
				enemigo_2(int(600), int(385),"villano",int(720)),
				enemigo(int(300), int(345),"villano",int(720)),
				enemigo_3(int(700), int(330),"villano",int(720)),
				enemigo(int(700), int(345),"villano",int(720))]
	...
	while esta_en_intro:
		...
		titulo = texto_intro.render('Alerta covid', 1, (255,0,0))
	...
	while esta_jugando:
		...
		villano[nivel].se_mueve_solo()

		# Verificar si colisiona el personaje_principal con perro cyborg

		if heroe.se_encuentra_con(villano[nivel]):
			heroe.es_golpeado()
			puntaje -= 5
			heroe.salud -= villano[nivel].daño
		...
		for bala in balas:
			if villano[nivel].se_encuentra_con(bala):
				bala.impacta_a(villano[nivel])
		...
		if villano[nivel].salud <= 0:
			subir_nivel()
			puntaje += villano[nivel].puntos
	...
	while final:
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				quit()
		ventana.fill((124, 252, 0)) # pinta el fondo de negro
		titulo = texto_intro.render('FIN DEL JUEGO', 1, (255,0,0))
		if gana:
			resultado = texto_resultado.render('HAS GANADO! B)', 1, (255,0,0))
		else:
			resultado = pygame.image.load("img/gameover.png")
		pts = texto_intro.render('Puntaje Total: '+str(puntaje), 1, (0,0,0))
		instrucciones = texto_intro.render('Presione ENTER para cerrar...', 1, (0,0,0))
		reintentar = texto_intro.render('Presione v para volver al juego...', 1, (0,0,0))
		ventana.blit(titulo, ((ventana_x//2)-titulo.get_width()//2, 10))
